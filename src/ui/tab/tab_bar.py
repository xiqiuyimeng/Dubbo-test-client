...
class MyTabBar(QTabBar):
    ...
    def eventFilter(self, obj: QObject, event: QEvent) -> bool:
        ...
        if obj == self and event.type() == QEvent.ToolTip:
            ...
            # 设置气泡提示
            self.setTabToolTip(index, str(self.property(self.tabText(index))))
    ...
    def handle_menu_func(self, act: QAction, index):
        ...
        if act.text() == CLOSE_CURRENT_TAB:
            ...
            [self.parent().removeTab(index + 1) for idx in range(self.count()) if idx > index]
            [self.parent().removeTab(0) for idx in range(self.count()) if idx < index]
        elif act.text() == CLOSE_ALL_TABS:
            # 删除所有tab
            [self.parent().removeTab(0) for idx in range(self.count())]
